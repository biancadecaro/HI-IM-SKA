...
HI_noise_maps_freq_nomask = file['maps_sims_HI'] + file['maps_sims_noise']
fg_maps_freq_nomask = file['maps_sims_fg']
full_maps_freq_nomask = file['maps_sims_tot'] + file['maps_sims_noise']
noise_maps_freq_nomask = file['maps_sims_noise']
print(HI_noise_maps_freq_nomask.shape)
...
fig.add_subplot(221) 
hp.mollview(full_maps_freq_nomask[ich], cmap='viridis', title=f'Observation', min=-1e3, max=1e4, hold=True)
fig.add_subplot(222) 
hp.mollview(HI_noise_maps_freq_nomask[ich], cmap='viridis', title=f'HI signal + noise',min=0, max=1,hold=True)
fig.add_subplot(223)
hp.mollview(fg_maps_freq_nomask[ich], title=f'Fg signal',cmap='viridis', min=-1e3, max=1e4, hold=True)
fig.add_subplot(224)
hp.mollview(noise_maps_freq_nomask[ich], title=f'Noise',cmap='viridis', hold=True)
plt.show()

del file

npix = np.shape(HI_noise_maps_freq_nomask)[1]
nside = hp.get_nside(HI_noise_maps_freq_nomask[0])
...
HI_noise_maps_freq = np.zeros((num_freq, npix))
fg_maps_freq = np.zeros((num_freq, npix))
full_maps_freq = np.zeros((num_freq, npix))

bad_v = np.where(mask_stripe==0)
...
out_dir = f'./Maps_needlets/No_mean/Beam_{beam_s}_noise_mask_patch_stripe82_2/'
