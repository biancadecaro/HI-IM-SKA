...
dir_GMCA = 'GMCA_maps/No_mean/Beam/'
out_dir_maps_recon = 'maps_reconstructed/No_mean/Beam/'
...
path_cosmo_HI = f'../GMCA_pixels_output/Maps_GMCA/No_mean/Beam/cosmo_HI_{num_ch}_{min_ch:1.1f}_{max_ch:1.1f}MHz'
path_fg = f'../GMCA_pixels_output/Maps_GMCA/No_mean/Beam/fg_input_synch_ff_ps_{num_ch}_{min_ch:1.1f}_{max_ch:1.1f}MHz'
...
path_cosmo_HI_bjk = f'../Maps_needlets/No_mean/Beam/bjk_maps_HI_{num_ch}freq_{min_ch:1.1f}_{max_ch:1.1f}MHz_jmax{jmax}_lmax{lmax}_B{B:1.2f}_nside{nside}'
path_input_fg_bjk = f'../Maps_needlets/No_mean/Beam/bjk_maps_fg_synch_ff_ps_{num_ch}freq_{min_ch:1.1f}_{max_ch:1.1f}MHz_jmax{jmax}_lmax{lmax}_B{B:1.2f}_nside{nside}'
#########################################################################################################
...
map_input_fg_need2pix=np.load(f'../PCA_needlets_output/maps_reconstructed/No_mean/Beam/maps_reconstructed_input_fg_{num_ch}_jmax{jmax}_lmax{lmax}_Nfg{Nfg}_nside{nside}.npy')
map_input_HI_need2pix=np.load(f'../PCA_needlets_output/maps_reconstructed/No_mean/Beam/maps_reconstructed_cosmo_HI_{num_ch}_jmax{jmax}_lmax{lmax}_Nfg{Nfg}_nside{nside}.npy')
...
fig.add_subplot(133) 
hp.gnomview(cosmo_HI[ich]-map_GMCA_HI_need2pix[ich], rot=[-22,21],coord='G', reso=hp.nside2resol(nside, arcmin=True), min=-0.2, max=0.2, title='HI-GMCA HI', cmap= 'viridis', hold=True)
plt.tight_layout()
plt.show()
...
fig.suptitle(f'channel: {nu_ch[ich]} MHz, jmax:{jmax}, lmax:{lmax}, Nfg:{Nfg}',fontsize=20)
fig.add_subplot(221) 
hp.mollview(100*(np.abs(map_GMCA_fg_need2pix[ich]/map_input_fg_need2pix[ich]-1)), min=0, max=0.2,  title= '(Res fg/fg-1)%',cmap='viridis',unit='%', hold= True)
fig.add_subplot(222) 
...
for n in range(len(nu_ch)):
    cl_cosmo_HI_recons[n] = hp.anafast(map_input_HI_need2pix[n], lmax=lmax_cl)#, use_pixel_weights=True)
    cl_cosmo_HI[n]=hp.anafast(cosmo_HI[n], lmax=lmax_cl)
    cl_GMCA_HI_need2harm[n] = hp.anafast(map_GMCA_HI_need2pix[n], lmax=lmax_cl)#, use_pixel_weights=True)
    cl_diff_cosmo_GMCA_HI_need2harm[n] = hp.anafast(cosmo_HI[n]-map_GMCA_HI_need2pix[n], lmax=lmax_cl)#, use_pixel_weights=True)

np.savetxt(out_dir_cl+f'cl_GMCA_HI_Nfg{Nfg}_lmax{lmax_cl}_nside{nside}.dat', cl_GMCA_HI_need2harm)
...
plt.plot(ell[1:], factor[1:]*cl_GMCA_HI_need2harm.mean(axis=0)[1:],'+',mfc='none', label = f'GMCA')
...
frame2=fig.add_axes((.1,.1,.8,.2))
plt.plot(ell[1:], diff_cl_need2sphe.mean(axis=0)[1:]*100, label='% GMCA_HI/input_HI -1')
