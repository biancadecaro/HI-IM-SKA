...
eigenvec_fg_Nfg = eigenvec[:, :,:num_sources]#eigenvec[:, :,Nfg:num_freq]#
...
for j in range(eigenvec_fg_Nfg.shape[0]):
    ...
    res_fg_maps = np.dot(eigenvec_fg_Nfg[j],np.dot(eigenvec_fg_Nfg[j].T,tot_map_wav))
...
cl_PCA_HI_need2harm_vecchio = np.loadtxt('PCA_needlets_output/maps_reconstructed/No_mean/Beam_theta40arcmin_noise/cls_recons_need/cl_PCA_HI_noise_synch_ff_ps_40_905_1295MHz_Nfg3_jmax4_lmax256_nside128.dat')
