...
###########################################################################
dir_PCA= '../PCA_mexican_needlets_output/maps_reconstructed/No_mean/p1/Beam_40arcmin/'
dir_GMCA = 'maps_reconstructed/No_mean/p1/Beam_40arcmin/'
...
cl_GMCA_HI = np.loadtxt(dir_GMCA_cl+'cl_GMCA_HI_Nfg3_jmax12_lmax512_nside256.dat')
...
fig=plt.figure()
fig.suptitle(f'mean over channels, Mex Need,\nbeam 40 arcmin, jmax:12, lmax:{lmax_cl}, Nfg:{Nfg}')
...
plt.legend()
plt.tight_layout()
plt.savefig(f'rel_diff_mean_cl_HI_PCA_GMCA_mex_need_jmax12_lmax{lmax_cl}_Nfg{Nfg}.png')
...
fig=plt.figure()
fig.suptitle(f'mean over channels, Mex Need,\nbeam 40 arcmin, jmax:12, lmax:{lmax_cl}, Nfg:{Nfg}')
plt.scatter(ell[2:], 100*np.mean(diff_PCA/diff_GMCA-1, axis=0)[2:])#,mfc='none')
plt.xlim([0,250])
plt.ylim([-20,20])
plt.xlabel(r'$\ell$')
plt.ylabel(r'$\% \langle \text{diff_PCA/diff_GMCA - 1} \rangle $')
plt.axhline(y=0,c='k',ls='--',alpha=0.5)
plt.tight_layout()
