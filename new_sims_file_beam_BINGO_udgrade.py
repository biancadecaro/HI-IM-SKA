...
def sigma_N(dnu, Tsys, nf, t_obs, theta_FWHM, Omega_sur): #tpix in sec and dnu in MHz
	t_pix = tpix(nf, t_obs, theta_FWHM, Omega_sur)
	return Tsys/np.sqrt(t_pix*dnu*1e6)

def tpix(nf, tobs, theta_FWHM, Omega_sur): #tobs in yrs
	Omega_pix = theta_FWHM**2
	tob_s = tobs*31536000 #s
	return nf*tob_s*(Omega_pix/Omega_sur)
...
idx_nu_min, = np.where(nu_ch==960.5)[0]
idx_nu_max, = np.where(nu_ch==1260.5)[0]
print(nu_ch[idx_nu_min], nu_ch[idx_nu_max])
...
delta_nu_out = 10
file_new['frequencies'] = nu_ch_f(nu_ch[idx_nu_min:idx_nu_max],delta_nu_out)

print(file_new['frequencies'])
...
for c in components:
  print(c)
  file_new[c]=merging_maps(nu_ch,file_new['frequencies'],file[c][idx_nu_min:idx_nu_max], delta_nu_out )
  file_ud[c] = hp.pixelfunc.ud_grade(map_in=file_new[c], nside_out=nside_out)

del file; del file_new

print(file_ud['frequencies'])
...
nf= 80 #feedhorns
Omega_sur     = 21000   # Survey area deg2
t_obs     = 1. # yrs, observing time
Tsys= 50 #K
theta_FWHM_arcmin = 40 #arcmin
theta_FWHM=theta_FWHM_arcmin*np.pi/(60*180) #rad
specs_dict = {'nf': nf, 'Tsys' : Tsys,
			  'Omega_sur': Omega_sur, 't_obs': t_obs, 'theta_FWHM' : theta_FWHM}
...
sigma_noise = sigma_N(dnu, **specs_dict)

noise = noise_map(sigma_noise,nside=nside_out) 
del sigma_noise
#########################################################################
...
for nu in range(num_freq_new):
		...
		del alm_HI
alm_noise = hp.map2alm(noise, lmax=lmax)
noise = hp.alm2map(alm_noise, lmax=lmax, nside = nside_out)
...
#########################################################################################
beam = hp.gauss_beam(theta_FWHM, lmax=lmax)
#beam =np.array( [hp.gauss_beam(theta_FWMH[i], lmax=lmax) for i in range(num_freq_new)])
lmax_fwmh = int(np.pi/theta_FWHM) 
print(f'theta_F at {np.min(nu_ch_new)} MHz:{theta_FWHM*180./np.pi:0.1f} deg, lmax_beam = {lmax_fwmh}\n')


file_sims_beam = {}
file_sims_beam['freq'] = nu_ch_new
file_sims_beam['maps_sims_tot'] =  np.array([convolve(obs_maps[i],beam, lmax=lmax) for i in range(num_freq_new)])
print('fatto 1 di 4')
file_sims_beam['maps_sims_fg'] =  np.array([convolve(fg_maps[i],beam, lmax=lmax) for i in range(num_freq_new)])
print('fatto 2 di 4')
file_sims_beam['maps_sims_HI'] =  np.array([convolve(file_ud['cosmological_signal'][i],beam, lmax=lmax) for i in range(num_freq_new)])
print('fatto 3 di 4')
file_sims_beam['maps_sims_noise'] =  noise#np.array([convolve(noise[i],beam, lmax=lmax) for i in range(num_freq_new)])
print('fatto 4 di 4')
...
for nu in range(num_freq_new):
		...
		del alm_obs
alm_noise = hp.map2alm(file_sims_beam['maps_sims_noise'], lmax=lmax)
file_sims_beam['maps_sims_noise'] = hp.alm2map(alm_noise, lmax=lmax, nside = nside_out)
#file_sims_beam['maps_sims_noise'][nu] = hp.remove_dipole(file_sims_beam['maps_sims_noise'][nu])
del alm_noise


filename = f'Sims/nuovo_beam_{theta_FWHM_arcmin}arcmin_BINGO_sims_{fg_comp}_noise_{len(nu_ch_new)}freq_{min(nu_ch_new)}_{max(nu_ch_new)}MHz_thick{dnu}MHz_lmax{lmax}_nside{nside_out}'
