...
npix = np.shape(fg_maps_freq)[1]
nside = hp.get_nside(fg_maps_freq[0])
lmax=3*nside-1

HI_maps_freq = pyfits.getdata(f'../GNILC/input/prior_seed_beam_{beam_s}_noise_savgol_100_2_maps_HI_{num_freq}freq_{min(nu_ch)}_{max(nu_ch)}_lmax{lmax}_nside{nside}.fits')


del file

hp.mollview(HI_maps_freq[0], cmap='viridis', title='HI smooth', min=0, max=1)
########################################################################################
#######################################
...
plt.show()
print(f'fsky50:{fsky_50},fsky50inv:{fsky_50_inv} ')
