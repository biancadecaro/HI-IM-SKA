...
need_dir = f'Maps_mexican_needlets_nuovo_1/No_mean/Beam_{beam_s}_noise_mask0.5_unseen/'
...
for j in range(eigenvec_fg_Nfg.shape[0]):
    ...
    leak_fg_maps[j] = need_fg_maps[:,j,:] - np.dot(eigenvec_fg_Nfg[j],np.dot(eigenvec_fg_Nfg[j].T,need_fg_maps[:,j,:]))
...
for j in range(eigenvec_fg_Nfg.shape[0]):
    ...
    leak_HI_maps[j] = np.dot(eigenvec_fg_Nfg[j],np.dot(eigenvec_fg_Nfg[j].T,need_HI_maps[:,j,:]))
