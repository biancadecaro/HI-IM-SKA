...
out_dir= 'PCA_pixels_output/Maps_PCA/No_mean/Beam_theta40arcmin_noise_mask0.20/'
out_dir_plot = 'PCA_pixels_output/Plots_PCA/No_mean/Beam_theta40arcmin_noise_mask0.20/'
...
mask1_20 = hp.read_map('HFI_Mask_GalPlane_2048_R1.10.fits', field=0)#fsky 20 % coverage
mask_20t = hp.ud_grade(mask1_20, nside_out=256)
mask_20 = hp.ud_grade(mask_20t, nside_out=nside)
del mask1_20
mask_20s = hp.sphtfunc.smoothing(mask_20, 3*np.pi/180,lmax=lmax) #apodization 3 deg come in Olivari
#del mask_40
fsky  = np.mean(mask_20s) 

fig=plt.figure()
hp.mollview(mask_20s, cmap='viridis', title=f'Mask smoothed, fsky={np.mean(mask_20s):0.2f}', hold=True)
plt.show()
...
for n in range(num_freq):
        ...
        HI_maps_freq[n] = HI_maps_freq[n]*mask_20s #- HI_maps_freq_mean
...
plt.plot(ell[2:], factor[2:]*cl_Hi[ich][2:],mfc='none', label='Cosmo HI+noise')
plt.plot(ell[2:], factor[2:]*cl_Hi_recons_Nfg[ich][2:],'+',mfc='none', label='PCA HI+noise')
...
fig=plt.figure()
plt.suptitle('Mean over channels')
plt.plot(ell[2:], 100*np.mean((cl_Hi-cl_Hi_recons_Nfg)/cl_Hi, axis=0)[2:],'--',mfc='none')
plt.xlabel(r'$\ell$')
