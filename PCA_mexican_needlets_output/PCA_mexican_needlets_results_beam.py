...
dir_PCA = 'PCA_maps/No_mean/p1/Beam_40arcmin/'
...
path_fg = f'../PCA_pixels_output/Maps_PCA/No_mean/Beam_40arcmin/fg_input_synch_ff_ps_{num_ch}_{min_ch:1.1f}_{max_ch:1.1f}MHz_lmax{lmax}_nside{nside}'
...
cmap= 'viridis'
fig.suptitle(f'MEX NEED, BEAM 40 arcmin, channel: {nu_ch[ich]} MHz, jmax:{jmax}, lmax:{lmax}, Nfg:{Nfg}',fontsize=19)
...
axs[1].set_title(f'Input HI + fg', fontsize=15)
...
plt.savefig(f'gnomview_HI_HIfg_PCAHI_mex_need_beam40arcmin_jmax{jmax}_lmax{lmax}_Nfg{Nfg}_nside{nside}.png')
...
plt.xlim([0,200])
plt.title(f'MEXICAN NEED CLs: BEAM mean over channels, jmax:{jmax}, lmax:{lmax_cl}, Nfg:{Nfg}')
plt.xlabel(r'$\ell$')
plt.ylabel(r'$ \frac{\ell*(\ell+1)}{2\pi} \langle C_{\ell} \rangle$')
plt.legend()
plt.tight_layout()
plt.savefig(f'recons_factorxcl_beam40arcmin_leakage_mex_p1_jmax{jmax}_lmax{lmax_cl}.png')
plt.show()
