...
###########################################################################3
path_data_sims_tot = 'Sims/beam_no_mean_sims_synch_ff_ps_40freq_905.0_1295.0MHz_nside256'
with open(path_data_sims_tot+'.pkl', 'rb') as f:
        file = pickle.load(f)
        f.close()

out_dir_output = 'GMCA_needlets_output/'
out_dir_output_GMCA = out_dir_output+'GMCA_maps/No_mean/Beam/'
out_dir_plot = out_dir_output+'Plots_GMCA_needlets/No_mean/Beam/'
...
need_dir = 'Maps_needlets/No_mean/Beam/'
...
#Ae = np.zeros((jmax, num_freq,num_sources))
...
np.save(out_dir_output_GMCA+f'leak_GMCA_{fg_comp}_jmax{jmax}_lmax{lmax}_{int(min(nu_ch))}_{int(max(nu_ch))}MHz_Nfg{num_sources}_nside{nside}.npy',leak_fg_maps)
...
del Ae; 
np.save(out_dir_output_GMCA+f'leak_GMCA_HI_{fg_comp}_jmax{jmax}_lmax{lmax}_{int(min(nu_ch))}_{int(max(nu_ch))}MHz_Nfg{num_sources}_nside{nside}.npy',leak_HI_maps)
...
plt.semilogy(ell[1:],factor[1:]*cl_GMCA_HI_need2harm[ich][1:],'+', mfc='none', label='GMCA HI')
