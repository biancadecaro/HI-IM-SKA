...
###########################################################################
beam_s = 'SKA_AA4'
out_dir= f'PCA_pixels_output/Maps_PCA/No_mean/Beam_{beam_s}_noise/'
out_dir_plot = f'PCA_pixels_output/Plots_PCA/No_mean/Beam_{beam_s}_noise/'
...
fig.add_subplot(222)
hp.mollview(fg_maps_freq[ich], cmap='viridis', title=f'Input foreground', hold=True)
fig.add_subplot(223)
hp.mollview(HI_maps_freq[ich], cmap='viridis', min=0, max=1,title=f'Input HI + noise', hold=True)
plt.show()
...
corr_coeff = np.corrcoef(full_maps_freq)
...
print(eigenvec_fg_Nfg.shape,eigenvec_fg_Nfg.T.shape,  full_maps_freq.shape)
...
np.save(out_dir+f'res_PCA_HI_noise_{fg_components}_{num_freq}_{min(nu_ch)}_{max(nu_ch)}MHz_Nfg{num_sources}_lmax{lmax}_nside{nside}.npy',res_HI)
...
np.save(out_dir+f'leak_PCA_HI_noise_{fg_components}_{num_freq}_{min(nu_ch)}_{max(nu_ch)}MHz_Nfg{num_sources}_lmax{lmax}_nside{nside}.npy',HI_leakage)
...
np.savetxt(out_dir_cl+f'cl_leak_HI_noise_{fg_components}_{num_freq}_{min(nu_ch)}_{max(nu_ch)}MHz_Nfg{num_sources}_lmax{lmax_cl}_nside{nside}.dat', cl_HI_leak_Nfg)
